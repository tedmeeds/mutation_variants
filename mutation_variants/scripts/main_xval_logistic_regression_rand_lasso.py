...
def main( X, normed_y, results_location, normalization, K, l1s, l2s, weakness, use_l1, seed ):
  if use_l1 is True:
    regs = l1s
  else:
    regs = l2s
  ...
  np.random.seed(seed)
  random_weights = weakness*np.random.rand(1,dim)
  ...
  for idx in range(len(regs)):
    ...
    for train_ids, test_ids in zip( tr_ids, te_ids ):
      ...
      if use_l1 is True:
        model = LogisticRegression( dim, l1=reg, weakness_weights = random_weights )
      else:
        model = LogisticRegression( dim, l2=reg, weakness_weights = random_weights )
